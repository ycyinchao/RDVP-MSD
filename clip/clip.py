...
# sm shape N_t
def similarity_map_to_points(sm, shape, t=0.8, down_sample=2,is_fg=True,bbox=None):
    ...
    # 如果提供了 bbox, 只在 bbox 区域内处理
    if bbox is not None:
        # 将 bbox 从原图的坐标系转换到 sm 的坐标系
        x_min, y_min, x_max, y_max = bbox
        scale_h = float(shape[0]) / sm.shape[0]  # 计算热图与原图的缩放比例
        scale_w = float(shape[1]) / sm.shape[1]

        # 映射 bbox 坐标到热图尺度
        x_min_sm = int(x_min / scale_w)
        y_min_sm = int(y_min / scale_h)
        x_max_sm = int(x_max / scale_w)
        y_max_sm = int(y_max / scale_h)

        # 判断是否 bbox是长宽是否为0（除了为零以外，还有可能是bbox的坐标超出了热图的范围，以及bbox过小，导致只有一个点满足情况）,所以需要重新初始化bbox
        if (x_max_sm -x_min_sm) * (y_max_sm - y_min_sm) <9:
            x_min_sm = 0
            y_min_sm = 0
            x_max_sm = sm.shape[1]
            y_max_sm = sm.shape[0]
        ...
        # 提取 sm_bbox（即 bbox 区域内的 sm）
        sm_bbox = sm[y_min_sm:y_max_sm, x_min_sm:x_max_sm]
        h,w = sm_bbox.shape

        # 对 sm_bbox 进行排序等处理
        sm_bbox = sm_bbox.reshape(-1)
        sm_bbox = (sm_bbox - sm_bbox.min()) / (sm_bbox.max() - sm_bbox.min())

        rank = sm_bbox.sort(0)[1]
        points_fg = []
        points_bg = []

        if is_fg:
            # 获取 bbox 内的前景和背景点
            num_fg = min((sm_bbox >= t).sum(), sm_bbox.shape[0] // 2)
            num_bg = min((sm_bbox <= (1 - t)).sum(), sm_bbox.shape[0] // 2)
            # 对于前景点，按提供的映射关系
            for idx in rank[-num_fg:]:
                # idx % w 得到的是列索引（热图中相对于 sm_bbox 的位置）
                # idx // w 得到的是行索引（热图中相对于 sm_bbox 的位置）
                x_sm = idx % w  # 相对于 sm_bbox 的列坐标
                y_sm = idx // w  # 相对于 sm_bbox 的行坐标
                # 映射到原图坐标系
                x = min((x_sm + x_min_sm) * scale_w, torch.tensor(shape[1] - 1))
                y = min((y_sm + y_min_sm) * scale_h, torch.tensor(shape[0] - 1))
                points_fg.append([int(x.item()), int(y.item())])

            # 对于背景点，按提供的映射关系
            for idx in rank[:num_bg]:
                x_sm = idx % w
                y_sm = idx // w
                x = min((x_sm + x_min_sm) * scale_w, torch.tensor(shape[1] - 1))
                y = min((y_sm + y_min_sm) * scale_h, torch.tensor(shape[0] - 1))
                points_bg.append([int(x.item()), int(y.item())])
            # 根据num去初始化label会存在问题，即num_fg和num_bg可能为0（好奇怪啊，我明明归一化了，按理说必定有个是1啊
            return points_fg, np.ones(num_fg).astype('uint8'), points_bg, np.zeros(num_bg).astype('uint8')
        else:
            # 获取 bbox 内的前景和背景点
            num_bg = min((sm_bbox >= t).sum(), sm_bbox.shape[0] // 2)
            num_fg = min((sm_bbox <= (1 - t)).sum(), sm_bbox.shape[0] // 2)
            # 对于背景点，按提供的映射关系
            for idx in rank[-num_bg:]:
                x_sm = idx % w
                y_sm = idx // w
        ...
        points_bg = []
        if is_fg:
            num_fg = min((sm >= t).sum(), sm.shape[0] // 2)
            num_bg = min((sm <= (1 - t)).sum(), sm.shape[0] // 2)
            labels_fg = np.ones(num_fg).astype('uint8')
            labels_bg = np.zeros(num_bg).astype('uint8')
            for idx in rank[-num_fg:]:
                x = min((idx % w) * scale_w, torch.tensor(shape[1] - 1))
                y = min((idx // w) * scale_h, torch.tensor(shape[0] - 1))
                points_fg.append([int(x.item()), int(y.item())])
            for idx in rank[:num_bg]:
                x = min((idx % w) * scale_w, torch.tensor(shape[1] - 1))
                y = min((idx // w) * scale_h, torch.tensor(shape[0] - 1))
                points_bg.append([int(x.item()), int(y.item())])
            return points_fg, labels_fg, points_bg, labels_bg
        else:
            num_fg = min((sm <= (1 - t)).sum(), sm.shape[0] // 2)
            num_bg = min((sm >= t).sum(), sm.shape[0] // 2)
            labels_fg = np.ones(num_fg).astype('uint8')
            labels_bg = np.zeros(num_bg).astype('uint8')
            for idx in rank[:num_fg]:
                x = min((idx % w) * scale_w, torch.tensor(shape[1] - 1))
